...
def get_list_elem(objects, tag, fd):
    """
    Return an XML element with the tag ``tag``, appending XML elements
    generated by calling ``get_elem(fd)`` for each member of
    ``objects``.
...
def read_tiles(elem, encoding=None, compression=None):
    """
...
def write_tiles(tiles, elem, encoding=None, compression=None):
    """
